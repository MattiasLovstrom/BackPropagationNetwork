...
namespace CodingBackProp
{
    public class NeuralNetwork
    {
        ...
        public override string ToString()
        {
            ...
            network.AppendLine($"Inputs:        {DisplayVector(inputs)}");
            for (var i = 0; i < numHidden; i++)
            {
                network.AppendLine($"Hidden input:  {DisplayVector(ihWeights[i])}");
                network.AppendLine($"Hidden output: {DisplayVector(hoWeights[i])}");
            }
            network.AppendLine($"Output:        {DisplayVector(outputs)}");

            return network.ToString();
        }

        private string DisplayVector(double[] doubles)
        {
            return "[" + doubles.Select(d => d.ToString("F4")).Aggregate((c, n) => c + ", " + n) + "]";
        }
        ...
        public double[] ComputeOutputs(double[] xValues)
        {
            ...
            for (int j = 0; j < numHidden; ++j)  // compute i-h sum of weights * inputs
                for (int i = 0; i < numInput; ++i)
                {
                    ...
                    Log($"  Sums[{j}] += inputs[{i}] * ihWeights[{i}][{j}] = {inputs[i]} * {ihWeights[i][j]} => hSums[{j}] = {hSums[j]}"); // note +=
                }

            for (int i = 0; i < numHidden; ++i)  // add biases to hidden sums
            {
                hSums[i] += hBiases[i];
                Log($"  hSums[{i}] += hBiases[{i}] => hSums[{i}] = {hSums[i]}");
            }
            ...
            for (int i = 0; i < numHidden; ++i)   // apply activation
            {
                ...
                Log($"  hOutputs[{i}] = HyperTan(hSums[{i}])) = {hOutputs[i]}"); // hard-coded
            }
            ...
            for (int i = 0; i < numHidden; ++i)
            {
                ...
                Log($"  oSums[{j}] += hOutputs[{j}] * hoWeights[{i}][{j}] => oSums[{j}] ={oSums[j]}");
            }
            ...
            {
                oSums[i] += oBiases[i];
                Log($"  oSums[{i}] += {oBiases[i]} => oSums[{i}] = {oSums[i]}");
            }
            ...
            msg = "[";
            foreach (var o in retResult) msg += o + " ";
            msg += "]";
            Log($"Output = {msg}");
            return retResult;
        }
        ...
        private static double[] Softmax(double[] oSums)
        {
            ...
            for (int i = 0; i < oSums.Length; ++i)
            {
                ...
                Log($"    sum += Math.Exp(oSums[{i}] => sum = {sum}");
            }
            ...
            for (int i = 0; i < oSums.Length; ++i)
            {
                ...
                Log($"    result[{i}] = Math.Exp(oSums[{i}]) / sum = {Math.Exp(oSums[i])} / {sum} = {result[i]}");
            }
          ...
          double learnRate, double momentum)
        {
            ...
            while (epoch < maxEpochs)
            {
                ...
                for (int ii = 0; ii < trainData.Length; ++ii)
                {
                    ...
                    Log("1. compute output node signals (assumes softmax)");
                    for (int k = 0; k < numOutput; ++k)
                    {
                        Log($"  o{k} = {outputs[k]}");
                        Log($"  target{k} = {tValues[k]}");

                        errorSignal = tValues[k] - outputs[k];  // Wikipedia uses (o-t)
                        Log($"  errorSignal = {tValues[k]} - {outputs[k]}  = {errorSignal}");  // Wikipedia uses (o-t)
                        derivative = (1 - outputs[k]) * outputs[k]; // for softmax
                        Log($"  derivative = (1 - {outputs[k]}) * {outputs[k]} = {derivative}"); // for softmax
                        oSignals[k] = errorSignal * derivative;
                        Log($"  oSignals[{k}] = errorSignal * derivative = {oSignals[k]}\n");
                    }

                    Log("2. compute hidden-to-output weight gradients using output signals");
                    for (int j = 0; j < numHidden; ++j)
                        for (int k = 0; k < numOutput; ++k)
                        {
                            hoGrads[j][k] = oSignals[k] * hOutputs[j];
                            Log($"  hoGrads[{j}][{k}] = oSignals[{k}] * hOutputs[{j}] = {oSignals[k]} * {hOutputs[j]} = {hoGrads[j][k]}");
                        }

                    Log("2b. compute output bias gradients using output signals");
                    for (int k = 0; k < numOutput; ++k)
                    {
                        obGrads[k] = oSignals[k] * 1.0; // dummy assoc. input value
                        Log($"  obGrads[{k}] = oSignals[{k}] * 1.0 = {obGrads[k]}"); // dummy assoc. input value
                    }

                    Log("3. compute hidden node signals");
                    for (int j = 0; j < numHidden; ++j)
                    {
                        derivative = (1 + hOutputs[j]) * (1 - hOutputs[j]); // for tanh
                        Log($"  derivative = (1 + hOutputs[{j}]) * (1 - hOutputs[{j}]) = (1 + {hOutputs[j]}) * (1 - {hOutputs[j]}) = {derivative}"); // for tanh
                        double sum = 0.0; // need sums of output signals times hidden-to-output weights
                        for (int k = 0; k < numOutput; ++k)
                        {
                            sum += oSignals[k] * hoWeights[j][k]; // represents error signal
                            Log($"  sum += oSignals[{k}] * hoWeights[{j}][{k}] = {oSignals[k]} * {hoWeights[j][k]} = {oSignals[k] * hoWeights[j][k]}"); // represents error signal
                        }
                        hSignals[j] = derivative * sum;
                        Log($"  hSignals[{j}] = derivative * sum ={hSignals[j]}");
                    }

                    Log("4. compute input-hidden weight gradients");
                    for (int i = 0; i < numInput; ++i)
                        for (int j = 0; j < numHidden; ++j)
                        {
                            ihGrads[i][j] = hSignals[j] * inputs[i];
                            Log($"  ihGrads[{i}][{j}] = hSignals[{j}] * inputs[{i}] = {ihGrads[i][j]}");
                        }

                    Log("4b. compute hidden node bias gradients");
                    for (int j = 0; j < numHidden; ++j)
                    {
                        hbGrads[j] = hSignals[j] * 1.0; // dummy 1.0 input
                        Log($"  hbGrads[{j}] = hSignals[{j}] * 1.0 = {hbGrads[j]}"); // dummy 1.0 input
                    }

                    // == update weights and biases

                    Log("update input-to-hidden weights");
                    for (int i = 0; i < numInput; ++i)
                    {
                        for (int j = 0; j < numHidden; ++j)
                        {
                            double delta = ihGrads[i][j] * learnRate;
                            Log($"  delta = ihGrads[{i}][{j}] * learnRate = {delta}");
                            ihWeights[i][j] += delta; // would be -= if (o-t)
                            Log($"  ihWeights[{i}][{j}] += delta"); // would be -= if (o-t)
                            ihWeights[i][j] += ihPrevWeightsDelta[i][j] * momentum;
                            Log($"  ihWeights[{i}][{j}] += ihPrevWeightsDelta[{i}][{j}] * momentum");
                            ihPrevWeightsDelta[i][j] = delta; // save for next time
                            Log($"  ihPrevWeightsDelta[{i}][{j}] = delta"); // save for next time
                        }
                    }

                    Log("update hidden biases");
                    for (int j = 0; j < numHidden; ++j)
                    {
                        double delta = hbGrads[j] * learnRate;
                        Log($"  delta = hbGrads[{j}] * learnRate");
                        hBiases[j] += delta;
                        Log($"  hBiases[{j}] += delta");
                        hBiases[j] += hPrevBiasesDelta[j] * momentum;
                        Log($"  hBiases[{j}] += hPrevBiasesDelta[{j}] * momentum");
                        hPrevBiasesDelta[j] = delta;
                        Log($"  hPrevBiasesDelta[{j}] = delta");
                    }

                    Log("update hidden-to-output weights");
                    for (int j = 0; j < numHidden; ++j)
                    {
                        for (int k = 0; k < numOutput; ++k)
                        {
                            double delta = hoGrads[j][k] * learnRate;
                            Log($"  delta = hoGrads[{j}][{k}] * learnRate");
                            hoWeights[j][k] += delta;
                            Log($"  hoWeights[{j}][{k}] += delta");
                            hoWeights[j][k] += hoPrevWeightsDelta[j][k] * momentum;
                            Log($"  hoWeights[{j}][{k}] += hoPrevWeightsDelta[{j}][{k}] * momentum");
                            hoPrevWeightsDelta[j][k] = delta;
                            Log($"  hoPrevWeightsDelta[{j}][{k}] = delta");
                        }
                    }

                    Log("update output node biases");
                    for (int k = 0; k < numOutput; ++k)
                    {
                        double delta = obGrads[k] * learnRate;
                        Log($"  delta = obGrads[{k}] * learnRate");
                        oBiases[k] += delta;
                        Log($"  oBiases[{k}] += delta");
                        oBiases[k] += oPrevBiasesDelta[k] * momentum;
                        Log($"  oBiases[{k}] += oPrevBiasesDelta[{k}] * momentum");
                        oPrevBiasesDelta[k] = delta;
                        Log($"  oPrevBiasesDelta[{k}] = delta");
                    }
                    //Console.Out.WriteLine(ToString());
                    Log("----------------------------------");
                } // each training item
